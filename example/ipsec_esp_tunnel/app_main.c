...
struct ofp_ipsec_config_param ipsec_cfg;
...
int main(int argc, char *argv[])
{
	...
	ofp_init_global_init(&app_init_params);
	...
	if (params.ipsec_async) {
		odp_queue_param_t qparam;

		ofp_ipsec_conf_param_init(&ipsec_cfg);
		ipsec_cfg.async_mode = 1;
		ipsec_cfg.async_queue_alloc = OFP_ASYNC_QUEUE_ALLOC_ROUNDROBIN;
		ipsec_cfg.async_queue_cnt = 1;
		...
		ipsec_cfg.async_queues[0] =
			odp_queue_create("async_queue", &qparam);
		if (ipsec_cfg.async_queues[0] == ODP_QUEUE_INVALID) {
			printf("Error: IPsec async queue creation failed.\n");
			ofp_term_local();
			ofp_term_global();
			odp_term_local();
			odp_term_global(instance);
		...
		app_init_params.ipsec_config = &ipsec_cfg;
	}
...
static int parse_args(int argc, char *argv[], appl_args_t *appl_args)
{
	...
	static struct option longopts[] = {
		...
		{"configuration file", required_argument,
			NULL, 'f'},/* return 'f' */
		{"ipsec asynchronous", no_argument, NULL, 'a'},	/* return 'a' */
		{NULL, 0, NULL, 0}
	};
	...
	while (1) {
		opt = getopt_long(argc, argv, "+c:i:hf:a",
				  longopts, &long_index);
