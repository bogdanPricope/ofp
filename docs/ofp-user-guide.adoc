...
- creating, configuration and deleting interfaces (ofp_ifnet_portconf.h)
...
the ofp_ifport_net_create() function. This will require the following steps:
...
- calling ofp_ifport_net_create() function for each interface
...
parameter structures to ofp_ifport_net_create() function (see the example in the
previous chapter).
